export default {
  async fetch(request, env) {
    ...
    if (request.method === 'POST' && url.pathname === '/api/orders') {
      ...
      if (!(data.confirmTransfer === true || data.confirmTransfer === 'on' || data.confirmTransfer === 'true' || data.confirmTransfer === 'true')) {
      ...
      const order = Object.assign({}, data, { received_at: new Date().toISOString(), status: 'pending' });
      ...
      return new Response(JSON.stringify({ success: true, orderId: id }), { status: 201, headers: { 'Content-Type': 'application/json' } });
    }
